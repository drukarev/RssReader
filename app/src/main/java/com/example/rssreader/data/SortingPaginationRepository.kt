...
class SortingPaginationRepository<T : Any, R : Any>(
    ...
    private suspend fun loadData(): ScreenViewModel<R> {
        ...
        val viewModel = when (state) {
            ...
            is State.End -> {
                ScreenViewModel.Data(state.loadedItems.map { formatFeedItem(it) }, hasMoreItems = false)
            }
        }
        ...
        return when (val page = repository.getPage(loadFromScratch = state is State.Start)) {
            is Response.Result -> {
                ...
                if (items.isEmpty()) {
                    ...
                    ScreenViewModel.Data(items = viewModelsWithProgress, hasMoreItems = hasMoreItems)
                }
            }
            is Response.Fail -> {
                if (items.isEmpty()) {
